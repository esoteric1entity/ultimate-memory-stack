...
- **Bash installer now writes `--extensions` selections to `PROFILE.md`.** `setup.sh` previously only echoed the activated extensions; `setup.py` already wrote them. Both doors now produce the same `PROFILE.md` extensions block.
