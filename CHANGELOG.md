...
### Fixed
- **The CI lock check claimed a verification it cannot perform — and the 4.0.1 notes repeated it.**
  The `addon-manifests` step echoed `OK (hash-verified)` and its comment promised that a tampered
  distribution "fails closed" — but under `--dry-run`, pip resolves from PEP 658 metadata and never
  downloads an artifact, so digest correctness is never checked. Negative-controlled 2026-08-24
  (pip 25.3, cold cache): a lock with a deliberately corrupted sha256 passed the exact CI command.
  The step's wording now states what it actually proves — the lock resolves and every line carries
  a hash — and names where digest enforcement really lives: the real installs in `addon-smoke`, and
  every user install, where a wrong digest fails closed and loud. The 4.0.1 CHANGELOG/README
  sentences repeating the overclaim are corrected in place, and the llmlingua smoke-test exclusion
  comment now names its cost honestly: llmlingua is the one add-on whose lock digests no CI job
  verifies. Found by negative-controlling the claim before first push — the same discipline
  `RELEASE.md` §0 prescribes, applied to the file that prescribes it.
...
A correctness release. Every pip add-on now installs — two of the three manifests were unresolvable in 4.0.0 — and the machinery that let that ship undetected is closed: CI now proves every manifest and lock resolves, and the installers place the files their own instructions tell users to run. Lint gains a real exit-code contract so a finding can fail a build instead of being printed and ignored.
