...
| `test_ge_setup.py` | `general-edition/setup.py` | preset/extension constants; `update_profile_*` regex edits; HMAC-secret generation; tier detection; **healthcare-preset refusal** branches (`SystemExit`); the `verify_environment` wizard-not-run branch |
