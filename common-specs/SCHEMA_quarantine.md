...
|-------|--------|---------------|
| Memory poisoning is a real attack class | OWASP `[OWASP-ASI06]`, arXiv:2503.03704 (MINJA) | OWASP standard + **arXiv preprint** (checked 2026-08-22: no `journal_ref` or DOI in its arXiv metadata, so we do not claim peer review — this entry previously said "peer-reviewed paper", which was the same overclaim we refuse to accept from vendor preprints elsewhere in these specs) |
