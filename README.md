...
[![Status: v4.0.1](https://img.shields.io/badge/Status-v4.0.1-green.svg)](#)
...
> **Compliance note:** HIPAA/PHI is out of scope for this edition.

---

## Project status

**v4.0.1 — current.** A correctness release on top of v4.0.0: every pip add-on now installs (two manifests were unresolvable), CI proves every manifest and hash-pinned lock resolves, and lint gained an exit-code contract so a finding can fail a build. v4.0.0 changed the installed layout, not just repo content: the protocol splits into a small always-loaded core plus an on-demand extended reference, `PROFILE.md` becomes a fully regenerable artifact with user configuration moved to `memory/user/USER_OVERRIDES.md`, and memory vaults gain a hot/cold tiered archive index. A one-command, non-destructive migration (with `--dry-run`) upgrades existing v3.6.x installs — see `general-edition/MIGRATION_v3.6_to_v4.0.md`. Predecessor versions (v3.0/v3.5/v3.6.0/v3.6.1/v3.6.2) have run in production on the maintainer's own machines since 2026-05-19, across Claude Code and OpenClaw deployments on three platforms, with a cross-machine validation cycle before v3.6.0 was cut.

- Build: production-ready
- Verification: two complementary layers — `verify.sh` validates an *install* (scaffold, registration, manifest), and `tests/` holds a **476-test pytest suite** *(as of 2026-08-24 — this figure rots; trust `pytest --collect-only` over this sentence)* covering the logic modules (lint runner, heartbeat compactor, edition setup, quarantine review, migration) plus the installer next-steps, the skill install-guard, the add-on lockfiles, and the citation registry. Platform-conditional tests skip where their platform's tooling is absent and run on the other OS's CI leg. Run them with `python -m pytest tests/`
- License: Apache-2.0
